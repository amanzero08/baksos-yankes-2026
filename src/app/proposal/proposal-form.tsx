          ...
          <CardContent className="pt-8 pb-8">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <Select onValueChange={(val: string | null) => setValue('committeeName', val || '')}>
                  <SelectTrigger className="bg-slate-900/50 border-white/10 text-slate-100 focus:ring-amber-500 h-12">
                    <SelectValue placeholder="Pilih nama Anda..." />
                  </SelectTrigger>
                  <SelectContent className="bg-slate-900 border-white/10 text-slate-200 max-h-[300px]">
                    {panitiaList.map((panitia: any) => (
                      <SelectItem key={panitia.id} value={panitia.committee_name}>
                        {panitia.committee_name}
                      </SelectItem>
                    ))}
                    {panitiaList.length === 0 && (
                      <div className="p-3 text-sm text-slate-500">Belum ada panitia yang terdaftar. Hubungi admin.</div>
                    )}
                  </SelectContent>
                </Select>
